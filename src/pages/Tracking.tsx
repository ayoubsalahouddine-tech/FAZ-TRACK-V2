...
const mapShipment = (row: ShipmentRow): ShipmentData => {
  ...
  return {
    ...
    sender: pickString(row, ['sender', 'shipper', 'expediteur', 'expéditeur']),
    recipient: pickString(row, ['recipient', 'consignee', 'destinataire']),
    ...
    shippingDate: formatDate(row.shipping_date ?? row.date_expedition ?? row.date_expédition),
...
export default function Tracking() {
  ...
  const reachedStep = useMemo(
    () => (shipment ? reachedStepByStatus[shipment.status] : -1),
    [shipment],
  )
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 mt-1">Suivez vos expéditions en temps réel.</p>
      </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
              <h3 className="text-lg font-semibold text-slate-800">Détails de l&apos;expédition</h3>
              <span
                className={`px-3 py-1 rounded-full text-xs font-semibold w-fit ${statusBadgeClass[shipment.status]}`}
              >
                {shipment.status}
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
              <DetailItem label="Numéro de suivi" value={shipment.trackingNumber} />
              <DetailItem label="Nom du client" value={shipment.clientName} />
              <DetailItem label="Expéditeur" value={shipment.sender} />
              ...
              <DetailItem label="Date d'expédition" value={shipment.shippingDate} />
              ...
              {timelineSteps.map((step, index) => {
                ...
                return (
                  <div key={step} className="flex items-start gap-3">
                    <div
                      className={`w-3 h-3 rounded-full mt-1 shrink-0 ${isReached ? 'bg-purple-600' : 'bg-slate-300'}`}
                    />
                    <p className={`text-sm font-medium ${isReached ? 'text-slate-800' : 'text-slate-400'}`}>
                      {step}
                    </p>
                  </div>
                )
